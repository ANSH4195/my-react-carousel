...
const MODAL_CONTAINER_PROPS = [
	...
	{
		attributeName: "class",
		value: "fixed inset-0 z-10 w-screen h-screen bg-gray-900 flex items-center px-4",
	},
];
...
const handleNavigate = (direction) => () => {
	...
	if (direction === "backward") {
		currentSlide =
			currentSlide === 0 ? SLIDES_LENGTH - 1 : currentSlide - 1;
	} else {
...
const changeVisibleSlide = (slideIndex) => {
	const carouselImageSelector =
		SELECTORS.carouselImage().querySelector("img");
	...
	carouselImageSelector.classList.add("animate-fade-left");
	setTimeout(() => {
		carouselImageSelector.classList.remove("animate-fade-left");
	}, 400);
};
...
const switchNavigationDot = (previousSlide, currentSlide) => {
	...
	const previousDot = dots[previousSlide];
	previousDot.innerHTML = UNFILLED_ICON;

	const currentDot = dots[currentSlide];
	currentDot.innerHTML = FILLED_ICON;
	currentDot.classList.add("animate-fade-up");
	setTimeout(() => {
		currentDot.classList.remove("animate-fade-up");
	}, 400);
};
...
(function attachRootEventListener() {
	SELECTORS.root().addEventListener("click", (event) => {
		const closestElementId = event.target
			.closest("[id]")
			.getAttribute("id");
